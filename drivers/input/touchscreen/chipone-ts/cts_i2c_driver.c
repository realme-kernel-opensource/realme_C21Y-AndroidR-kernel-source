...
static int __init lcd_name_get(char *str)
{
        if (str != NULL)
                lcd_name = str;
        cts_info("lcd name from uboot: %s\n", lcd_name);
...
static int __init cts_driver_init(void)
{
   ...
   if (!(strncmp(lcd_name,"lcd_icnl9911c_boe_mipi_hd",strlen("lcd_icnl9911c_boe_mipi_hd")))) {
		cts_tpmodule = 0;
        cts_info("This is lcd_icnl9911c_boe_mipi_hd\n");
    } else if (!(strncmp(lcd_name,"lcd_icnl9911c_truly_mipi_hd",strlen("lcd_icnl9911c_truly_mipi_hd")))) {
		cts_tpmodule = 1;
