...
{
	seq_printf(s, "0 errors. All test passed.\n");
								  ...
								  size_t len, loff_t *pos)
{
	size_t ret = 0;
	char *temp_buf;
	struct irq_desc *desc = irq_to_desc(private_ts->hx_irq);//wdd modify
...
static ssize_t proc_limit_area_read(struct file	*file, char	__user *user_buf,	size_t count,	loff_t *ppos)
{
	...
	ret	=	snprintf(page, PAGESIZE,"limit_area=%d left_x1=%d right_x1=%d left_x2=%d right_x2=%d left_y1=%d right_y1=%d left_y2=%d right_y2=%d\n",
...
static void himax_notifie_hwreset_workqueue(struct work_struct *work)
{
...
{
	schedule_work(&private_ts->hwreset_work_queue);
	return;
}
...
static int i_get_FW(void)
{
	...
	if(!private_ts->recovery_mode){
		// ret = request_firmware_select(&hxfw, hx_fw->firmware_sign_name, private_ts->dev);
			ret = request_firmware(&hxfw, hx_fw->firmware_sign_name, private_ts->dev);
		 if (ret < 0) {
...
static void himax_excp_hw_reset(void)
{
	...
	I("%s: It will update fw after exception event in zero flash mode!\n",
		__func__);
	result = g_core_fp.fp_0f_op_file_dirly(hx_fw->firmware_name);
	if (result) {
...
static void himax_boot_upgrade(struct work_struct *work)
{
	...
	tp_register_devinfo(ic_data->vendor_touch_cfg_ver);
	if (fw_sts == NO_ERR)
	{
#ifdef OPLUS_PROC_NODE
		if(!private_ts->recovery_mode){
...
#ifdef SPRD_SYSFS_SUSPEND_RESUME
...
int himax_chip_common_init(void)
{
	...
	ts->boot_mode = mode_name;//get_boot_mode();
	I("==========ts->boot_mode = %d\n",ts->boot_mode);
#ifdef CONFIG_TOUCHPANEL_MTK_PLATFORM
    ...
    if ((ts->boot_mode == MSM_BOOT_MODE__FACTORY || ts->boot_mode == MSM_BOOT_MODE__RF || ts->boot_mode == MSM_BOOT_MODE__WLAN 
		|| ts->boot_mode == MSM_BOOT_MODE__CHARGE || ts->boot_mode == MSM_BOOT_MODE__CALI))
#endif
	{
 ...
 */
	...
	INIT_WORK(&ts->hwreset_work_queue, himax_notifie_hwreset_workqueue);
