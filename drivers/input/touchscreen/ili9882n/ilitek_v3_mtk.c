...
#define DTS_OF_NAME	"ilitek,ili9882n"
...
static int ilitek_plat_probe(void)
{
	...
	ilits->boot_mode = get_boot_mode();
	ILI_INFO("==========idev->boot_mode = %d\n",ilits->boot_mode);
	if ((ilits->boot_mode == 3 || ilits->boot_mode == 4 || ilits->boot_mode == 5)) {
		ILI_INFO("boot_mode is FACTORY,RF and WLAN not need to enable irq\n");
		ili_irq_disable();
	}
...
static int __parse_tag_videolfb(struct device_node *node)
{
	...
	if (!(strncmp(mtkfb_lcm_name,"ilt9882n_ls_psc_hdp_dsi_vdo_lcm",strlen("ilt9882n_ls_psc_hdp_dsi_vdo_lcm")))) {
		ili_tp_module = MODEL_LS;
		ILI_INFO("this is ili9882n_ls ilitek touchscreen\n");
	} else if (!(strncmp(mtkfb_lcm_name,"ilt9882n_truly_psc_hdp_dsi_vdo_lcm",strlen("ilt9882n_truly_psc_hdp_dsi_vdo_lcm")))) {
