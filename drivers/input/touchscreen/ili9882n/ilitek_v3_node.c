...
unsigned char ili9882_user_buf[USER_STR_BUFF] = {0};
...
static ssize_t ilitek_proc_get_delta_data_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	if (copy_to_user(buf, ili9882_user_buf, len))
...
static ssize_t ilitek_proc_fw_pc_counter_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	len = snprintf(ili9882_user_buf, PAGE_SIZE, "pc = 0x%x, latch = 0x%x\n", ilits->fw_pc, ilits->fw_latch);
...
static ssize_t ilitek_proc_rw_tp_reg_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	if (copy_to_user((char *)buf, ili9882_user_buf, len))
		ILI_ERR("Failed to copy data to user space\n");
...
static ssize_t ilitek_proc_debug_switch_read(struct file *pFile, char __user *buff, size_t size, loff_t *pos)
{
	...
	size = snprintf(ili9882_user_buf, USER_STR_BUFF * sizeof(unsigned char), "dnp : %s\n", ilits->dnp ? "Enabled" : "Disabled");
	*pos = size;

	if (copy_to_user(buff, ili9882_user_buf, size))
		ILI_ERR("Failed to copy data to user space\n");
...
static ssize_t ilitek_node_mp_lcm_on_test_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	ili9882_user_buf[0] = 3;
	ili9882_user_buf[1] = (ret < 0) ? -ret : ret;
	len += ilits->mp_ret_len;

	len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "MP TEST %s\n", (ret < 0) ? "FAIL" : "PASS");
	if (ili9882_user_buf[1] == EMP_MODE) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Failed to switch MP mode, abort!");
	} else if (ili9882_user_buf[1] == EMP_FW_PROC) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "FW still upgrading, abort!");
	} else if (ili9882_user_buf[1] == EMP_FORMUL_NULL) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "MP formula is null, abort!");
	} else if (ili9882_user_buf[1] == EMP_INI) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Not found ini file, abort!");
	} else if (ili9882_user_buf[1] == EMP_NOMEM) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Failed to allocated memory, abort!");
	} else if (ili9882_user_buf[1] == EMP_PROTOCOL) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Protocol version isn't matched, abort!");
	} else if (ili9882_user_buf[1] == EMP_TIMING_INFO) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Failed to get timing info, abort!");
	} else if (ili9882_user_buf[1] == EMP_PARA_NULL) {
...
static ssize_t ilitek_node_ver_info_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "CHIP ID = %x\n", ilits->chip->id);
	len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "FW version = %d.%d.%d.%d\n",
			ilits->chip->fw_ver >> 24, (ilits->chip->fw_ver >> 16) & 0xFF,
			(ilits->chip->fw_ver >> 8) & 0xFF, ilits->chip->fw_ver & 0xFF);
	len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "FW MP version = %d.%d.%d.%d\n",
	...
	if (ilits->protocol->core_ver_len == P5_X_CORE_VER_THREE_LENGTH)
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "Core version = %d.%d.%d\n",
			(ilits->chip->core_ver >> 24) & 0xFF, (ilits->chip->core_ver >> 16) & 0xFF,
			(ilits->chip->core_ver >> 8) & 0xFF);
	else
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "Core version = %d.%d.%d.%d\n",
			(ilits->chip->core_ver >> 24) & 0xFF, (ilits->chip->core_ver >> 16) & 0xFF,
			(ilits->chip->core_ver >> 8) & 0xFF, ilits->chip->core_ver & 0xFF);

	len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "Protocol version = %d.%d.%d\n",
			ilits->protocol->ver >> 16, (ilits->protocol->ver >> 8) & 0xFF,
			ilits->protocol->ver & 0xFF);
	len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "Driver version = %s\n", DRIVER_VERSION);
...
static ssize_t ilitek_node_change_list_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "============= Change list ==============\n");
	len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "[Drive version] = %s\n", DRIVER_VERSION);
	// len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "[Patch] 202001-0001\n");
	len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "========================================\n");
...
static ssize_t ilitek_proc_fw_process_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	len = snprintf(ili9882_user_buf, USER_STR_BUFF * sizeof(unsigned char), "%02d\n", ilits->fw_update_stat);
...
static ssize_t ilitek_node_fw_upgrade_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	ili9882_user_buf[0] = 0x0;
	ili9882_user_buf[1] = (ret < 0) ? -ret : ret;

	if (ili9882_user_buf[1] == 0)
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Upgrade firmware = PASS");
	else
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Upgrade firmware = FAIL");

	/* Reason for fail */
	if (ili9882_user_buf[1] == EFW_CONVERT_FILE) {
		ili9882_user_buf[0] = 0xFF;
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Failed to convert hex/ili file, abort!");
	} else if (ili9882_user_buf[1] == ENOMEM) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Failed to allocate memory, abort!");
	} else if (ili9882_user_buf[1] == EFW_ICE_MODE) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Failed to operate ice mode, abort!");
	} else if (ili9882_user_buf[1] == EFW_CRC) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "CRC not matched, abort!");
	} else if (ili9882_user_buf[1] == EFW_REST) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Failed to do reset, abort!");
	} else if (ili9882_user_buf[1] == EFW_ERASE) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Failed to erase flash, abort!");
	} else if (ili9882_user_buf[1] == EFW_PROGRAM) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Failed to program flash, abort!");
	} else if (ili9882_user_buf[1] == EFW_INTERFACE) {
		len += snprintf(ili9882_user_buf + len, USER_STR_BUFF - len, "%s\n", "Failed to hex file interface no match, abort!");
	}

	if (copy_to_user((u32 *) buff, ili9882_user_buf, len))
		ILI_ERR("Failed to copy data to user space\n");
...
static ssize_t ilitek_proc_debug_level_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	size = snprintf(ili9882_user_buf, USER_STR_BUFF * sizeof(unsigned char), "debug level : %s\n", debug_en ? "Enable" : "Disable");

	*pos += size;

	if (copy_to_user((u32 *) buff, ili9882_user_buf, size))
		ILI_ERR("Failed to copy data to user space\n");
