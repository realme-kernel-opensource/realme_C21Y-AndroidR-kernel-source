...
static int __init lcd_name_get(char *str)
{
        if (str != NULL)
                lcd_name = str;
        ILI_INFO("lcd name from uboot: %s\n", lcd_name);
...
static int __init ilitek_plat_dev_init(void)
{
	...
	if (!(strncmp(lcd_name,"ilt9882n_ls_psc_hdp_dsi_vdo_lcm",strlen("ilt9882n_ls_psc_hdp_dsi_vdo_lcm")))) {
		ili_tp_module = MODEL_LS;
		ILI_INFO("this is ili9882n_ls ilitek touchscreen\n");
	} else if (!(strncmp(lcd_name,"ilt9882n_truly_psc_hdp_dsi_vdo_lcm",strlen("ilt9882n_truly_psc_hdp_dsi_vdo_lcm")))) {
		ili_tp_module = MODEL_TRULY;
		ILI_INFO("this is ili9882n_ls ilitek touchscreen\n");
	} else if (!(strncmp(lcd_name,"lcd_ili9882q_skyworth_mipi_hd",strlen("lcd_ili9882q_skyworth_mipi_hd")))) {
		ili_tp_module = MODEL_SKY;
		ILI_INFO("this is ili9882n_ls ilitek touchscreen\n");
	} else if (!(strncmp(lcd_name,"lcd_ili7806s_txd_mipi_hd",strlen("lcd_ili7806s_txd_mipi_hd")))) {
