...
/*ritchie add for oplus*/
	...
	ilits->pre_x = touch_info[0].x;
	ilits->pre_y = touch_info[0].y;/*ritchie add for oplus*/
#ifdef ROI
	...
	if (ilits->finger) {
		...
		if (ilits->last_touch) {
			...
			input_sync(ilits->input);
			ilits->last_touch = 0;
			OPLUS_DBG(1, "release [x,y] = %d, %d\n", ilits->pre_x, ilits->pre_y);/*ritchie add for oplus*/
		}
		ilits->first_report = true;/*ritchie add for oplus*/
	}
...
int ili9882n_get_gesture_info(u8 *buf, struct gesture_coordinate * gesture)/*ilits->gcoord*/
{
    ...
    switch (gesture_id)     //judge gesture type
    {
        case GESTURE_RIGHT :
			...
			gesture->pos_1st.x   = 0;  //ymain
            gesture->pos_1st.y   = 0;
            gesture->pos_2nd.x   = 0;  //xmin
            gesture->pos_2nd.y   = 0;
            gesture->pos_3rd.x   = 0;  //ymax
            gesture->pos_3rd.y   = 0;
            gesture->pos_4th.x   = 0;  //xmax
            gesture->pos_4th.y   = 0;
			gesture->clockwise     = 1;

            gesture->pos_start.x = (((point_data[4] & 0xF0) << 4) | (point_data[5]));
            gesture->pos_start.y = (((point_data[4] & 0x0F) << 8) | (point_data[6]));
            gesture->pos_end.x   = (((point_data[7] & 0xF0) << 4) | (point_data[8]));
            gesture->pos_end.y   = (((point_data[7] & 0x0F) << 8) | (point_data[9]));
            break;
        ...
        case GESTURE_DOUBLECLICK:
            ...
            gesture->pos_start.x = (((point_data[4] & 0xF0) << 4) | (point_data[5]));
            gesture->pos_start.y = (((point_data[4] & 0x0F) << 8) | (point_data[6]));
            gesture->pos_end     = gesture->pos_start;
            break;
        ...
        case GESTURE_V :
            ...
            gesture->pos_end.x   = (((point_data[7] & 0xF0) << 4) | (point_data[8]));
            gesture->pos_end.y   = (((point_data[7] & 0x0F) << 8) | (point_data[9]));
            gesture->pos_1st.x   = (((point_data[16] & 0xF0) << 4) | (point_data[17]));
            gesture->pos_1st.y   = (((point_data[16] & 0x0F) << 8) | (point_data[18]));
            break;
        ...
        case GESTURE_O  :
            ...
            gesture->pos_1st.x   = ((rd_x + lu_x) / 2);  //ymain
            gesture->pos_1st.y   = lu_y;
            gesture->pos_2nd.x   = lu_x;  //xmin
            gesture->pos_2nd.y   = ((rd_y + lu_y) / 2);
            gesture->pos_3rd.x   = ((rd_x + lu_x) / 2);  //ymax
            gesture->pos_3rd.y   = rd_y;
            gesture->pos_4th.x   = rd_x;  //xmax
            gesture->pos_4th.y   = ((rd_y + lu_y) / 2);
            break;
        ...
        case GESTURE_M  :
            ...
            gesture->pos_1st.x   = (((point_data[16] & 0xF0) << 4) | (point_data[17]));
            gesture->pos_1st.y   = (((point_data[16] & 0x0F) << 8) | (point_data[18]));
            gesture->pos_2nd.x   = (((point_data[19] & 0xF0) << 4) | (point_data[20]));  //xmin
            gesture->pos_2nd.y   = (((point_data[19] & 0x0F) << 8) | (point_data[21]));
            gesture->pos_3rd.x   = (((point_data[22] & 0xF0) << 4) | (point_data[23]));  //ymax
            gesture->pos_3rd.y   = (((point_data[22] & 0x0F) << 8) | (point_data[24]));
            break;
		...
		case GESTURE_TWOLINE_DOWN :
            ...
            gesture->pos_1st.x   = (((point_data[10] & 0xF0) << 4) | (point_data[11]));
            gesture->pos_1st.y   = (((point_data[10] & 0x0F) << 8) | (point_data[12]));
            gesture->pos_2nd.x   = (((point_data[13] & 0xF0) << 4) | (point_data[14]));
            gesture->pos_2nd.y   = (((point_data[13] & 0x0F) << 8) | (point_data[15]));
            break;
 	...
 	ILI_DBG("gesture data 0-17 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X\n", \
	...
	ILI_DBG("gesture data 18-35 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X\n", \
	...
	ILI_INFO("gesture debug data 160-168 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X\n", \
	...
	ILI_DBG("before scale gesture_id: 0x%x, score: %d, gesture_type: %d, clockwise: %d, points: (%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)\n", \
    ...
    if (!transfer) {
		gesture->pos_start.x = gesture->pos_start.x * ilits->panel_wid / TPD_WIDTH;
		gesture->pos_start.y = gesture->pos_start.y * ilits->panel_hei / TPD_HEIGHT;
		gesture->pos_end.x = gesture->pos_end.x * ilits->panel_wid / TPD_WIDTH;
		gesture->pos_end.y = gesture->pos_end.y * ilits->panel_hei / TPD_HEIGHT;
		gesture->pos_1st.x = gesture->pos_1st.x * ilits->panel_wid / TPD_WIDTH;
		gesture->pos_1st.y = gesture->pos_1st.y * ilits->panel_hei / TPD_HEIGHT;

		gesture->pos_2nd.x = gesture->pos_2nd.x * ilits->panel_wid / TPD_WIDTH;
		gesture->pos_2nd.y = gesture->pos_2nd.y * ilits->panel_hei / TPD_HEIGHT;

		gesture->pos_3rd.x = gesture->pos_3rd.x * ilits->panel_wid / TPD_WIDTH;
		gesture->pos_3rd.y = gesture->pos_3rd.y * ilits->panel_hei / TPD_HEIGHT;

		gesture->pos_4th.x = gesture->pos_4th.x * ilits->panel_wid / TPD_WIDTH;
		gesture->pos_4th.y = gesture->pos_4th.y * ilits->panel_hei / TPD_HEIGHT;
	}
    ILI_INFO("gesture_id: 0x%x, score: %d, gesture_type: %d, clockwise: %d, points: (%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)(%d, %d)\n", \
                gesture_id, score, gesture->gesture_type, gesture->clockwise, \
                gesture->pos_start.x, gesture->pos_start.y, \
                gesture->pos_end.x, gesture->pos_end.y, \
                gesture->pos_1st.x, gesture->pos_1st.y, \
