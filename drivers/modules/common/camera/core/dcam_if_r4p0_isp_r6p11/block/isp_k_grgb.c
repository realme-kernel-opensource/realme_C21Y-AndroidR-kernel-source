...
static int isp_k_grgb_block(struct isp_io_param *param, enum isp_id idx)
{
	...
	ret = copy_from_user((void *)&grgb_info, param->property_param,
				sizeof(grgb_info));
	if (ret != 0) {
	...
	if (grgb_info.bypass) {
		isp_dbg_s_ori_byp(SBLK_BYPASS, raw_grgbc, idx);
		return 0;
	}
		...
		struct isp_k_block *isp_k_param, enum isp_id idx)
{
	...
	if (!param) {
		pr_err("fail to get invalid param.\n");
		return -EPERM;
	}

	if (param->property_param == NULL) {
		pr_err("fail to get invalid property_param.\n");
		return -EPERM;
	}
