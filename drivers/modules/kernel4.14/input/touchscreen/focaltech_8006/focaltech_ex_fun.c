...
#if defined(WW6_DRV_TP_FW_AUTO_UPGRADE)

#if defined(WW6_DRV_TP_FW_FILE_I)
static unsigned char CTPM_FW[] = {
#include WW6_DRV_TP_FW_FILE_I
};
#else
#error "WW6_DRV_TP_FW_FILE_I has to be defined for focaltech tp upgrace"
#endif

#elif defined(CONFIG_TOUCHSCREEN_FIRMWARE_UPGRADE)
static unsigned char CTPM_FW[] = {
...
int fts_write_reg_ww(struct i2c_client *client, u8 regaddr, u8 regvalue)
{
...
int fts_read_reg_ww(struct i2c_client *client, u8 regaddr, u8 *regvalue)
{
...
int fts_ctpm_auto_clb(struct i2c_client *client)
{
	...
	fts_write_reg_ww(client, 0, FTS_FACTORYMODE_VALUE);
	/*make sure already enter factory mode */
	msleep(100);
	/*write command to start calibration */
	fts_write_reg_ww(client, 2, 0x4);
	msleep(300);
	if ((fts_updateinfo_curr.CHIP_ID==0x11) ||(fts_updateinfo_curr.CHIP_ID==0x12) ||(fts_updateinfo_curr.CHIP_ID==0x13) ||(fts_updateinfo_curr.CHIP_ID==0x14)) //5x36,5x36i
	{
		for(i=0;i<100;i++)
		{
			fts_read_reg_ww(client, 0x02, &uc_temp);
			if (0x02 == uc_temp ||
				0xFF == uc_temp)
			{
		...
		for(i=0;i<100;i++)
		{
				fts_read_reg_ww(client, 0, &uc_temp);
	...
	/*calibration OK*/
	fts_write_reg_ww(client, 0, 0x40);  /*goto factory mode for store*/
	msleep(200);   /*make sure already enter factory mode*/
	fts_write_reg_ww(client, 2, 0x5);  /*store CLB result*/
	msleep(300);
	fts_write_reg_ww(client, 0, FTS_WORKMODE_VALUE);	/*return to normal mode */
	msleep(300);
...
int fts_ctpm_update_project_setting(struct i2c_client *client)
{
	...
	*/
	...
	{
		fts_write_reg_ww(client, 0xbc, 0xaa);
	}
	else fts_write_reg_ww(client, 0xfc, 0xaa);
	msleep(50);

	/*write 0x55 to register 0xfc */
	if(fts_updateinfo_curr.CHIP_ID==0x06 || fts_updateinfo_curr.CHIP_ID==0x36)
	{
		fts_write_reg_ww(client, 0xbc, 0x55);
	}
	else fts_write_reg_ww(client, 0xfc, 0x55);
	msleep(30);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	fts_read_reg_ww(client, FT5X0X_REG_CIPHER, &uc_reg_value);
...
#endif
	fts_read_reg_ww(client, FT_REG_FW_VER, &uc_tp_fm_ver);
	uc_host_fm_ver = fts_ctpm_get_i_file_ver();

       printk("[FTS]chenqx uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);
#if !defined(WW6_FT6XXX_HARD_UPGRADE_FW)
...
//	int i_ret;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		fts_write_reg_ww(client, 0xbc, FT_UPGRADE_AA);
		msleep(fts_updateinfo_curr.delay_aa);

		/*write 0x55 to register 0xbc */
		fts_write_reg_ww(client, 0xbc, FT_UPGRADE_55);
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		/*write 0xaa to register 0xfc*/
		/*write 0xaa to register 0xfc*/
		fts_write_reg_ww(client, 0xfc, FT_UPGRADE_AA);
		msleep(fts_updateinfo_curr.delay_aa);

		 /*write 0x55 to register 0xfc*/
		fts_write_reg_ww(client, 0xfc, FT_UPGRADE_55);
		msleep(fts_updateinfo_curr.delay_55);
...
int hid_to_i2c(struct i2c_client * client)
{
	...
	ret = fts_i2c_Read(client, auc_i2c_write_buf, 0, auc_i2c_write_buf, 3);
	if(ret < 0){
					...
					char *buf)
{
	...
	if (fts_read_reg_ww(client, FT_REG_FW_VER, &fwver) < 0)
					...
					const char *buf, size_t count)
{
	...
	if (2 == num_read_chars) {
		/*read register*/
		regaddr = wmreg;
		if (fts_read_reg_ww(client, regaddr, &regvalue) < 0)
		...
		regaddr = wmreg >> 8;
		regvalue = wmreg;
		if (fts_write_reg_ww(client, regaddr, regvalue) < 0)
...
int fts_create_sysfs_ww(struct i2c_client *client)
{
	int err;
...
static ssize_t fts_debug_read(struct file *filp, char __user *page, size_t len, loff_t *pos)
{
	...
	case PROC_UPGRADE:
		...
		regaddr = 0xA6;
		ret = fts_read_reg_ww(client, regaddr, &regvalue);
		if (ret < 0)
