...
int idc_esdcheck_lcderror(struct fts_ts_data *ts_data)
{
	...
	if (val == 0xAA) {
		 ...
		 * 2. LCD driver need reset(recovery)
		 *    LCD and set lcd_need_reset to 0
		 * 3. recover TP state
		 */
...
static int esdcheck_algorithm(struct fts_ts_data *ts_data)
{
	...
	if (fts_esdcheck_data.suspend == 1) {
		...
		/* because in suspend state, adb can be used, when upgrade
		 * FW, will active ESD check(active = 1); But in suspend
		 * then will don't queue_delayed_work, when resume, don't
		 * check ESD again.
		 */
		return 0;
	}
	...
	/* 7. get Flow work cnt: 0x91 If no change
	 *    for 5 times, then ESD and reset
	 */
	if (!hardware_reset)
 ...
 *
 */
static DEVICE_ATTR(fts_esd_mode, 0644, fts_esdcheck_show,
		   fts_esdcheck_store);
