...
static int fts_wait_idle(void)
{
	...
	if (i >= retry_timeout) {
		FTS_ERROR("spi is busy, status:0x%x, rxbuf:%d", status,
			  (int)rxbuf[CD_PACKAGE_BUFLEN]);
...
static int fts_cmdpkg_write(u8 ctrl, u8 *cmd, u32 cmdlen)
{
	int i = 0;
	u8 *txbuf = fts_data->bus_tx_buf;
	u8 *rxbuf = fts_data->bus_rx_buf;
...
static int fts_boot_write(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
	...
	if (ts_data->fw_is_running && data && datalen) {
		...
		fts_cmdpkg_write(WRITE_CMD, tmpcmd, cmdlen);
	}
	ret = fts_cmdpkg_write(WRITE_CMD, tmpcmd, cmdlen);
	if (ret < 0) {
		FTS_ERROR("command package write fail");
		goto err_boot_write;
	}
	...
	if (data && datalen) {
		...
		ret = fts_spi_transfer(txbuf, rxbuf, txlen);
		if (ret < 0)
			FTS_ERROR("spi_transfer(write) fail");
	}
...
int fts_read(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
	...
	if (cmd && cmdlen) {
		...
		ret = fts_cmdpkg_write(ctrl, tmpcmd, cmdlen);
		if (ret < 0) {
			FTS_ERROR("command package write fail");
			goto boot_read_err;
		}
	...
	if (data && datalen) {
		...
		if (ctrl & DATA_CRC_EN) {
			...
			if (ret < 0) {
				FTS_INFO(
					 "read data(addr:%x) crc check incorrect,in %s mode",
					cmdaddr,
					ts_data->fw_is_running ? "fw" : "boot");
				goto boot_read_err;
			}
		}
	}
