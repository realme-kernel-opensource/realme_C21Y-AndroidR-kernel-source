		...
		struct device_node *dev_node)
{
	...
	pdata->int_gpio = of_get_gpio(dev_node, 1);
	...
	pdata->rst_gpio = of_get_gpio(dev_node, 0);
...
int cts_plat_release_all_touch(struct cts_platform_data *pdata)
{
    ...
    input_sync(input_dev);
#ifdef CFG_CTS_FORCE_UP
    WARN_ON(del_timer(&pdata->touch_event_timeout_timer) != 0);
#endif
    return 0;
}
