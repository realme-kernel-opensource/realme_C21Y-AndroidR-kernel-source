...
#define SYSTEM_WIFI_612CE_CONFIG_FILE "/vendor/etc/wifi_board_612ceconfig.ini"
...
int get_wifi_config_param(struct wifi_conf_t *p)
{
    ...
    if(strstr(saved_command_line,"board_id=S19612AA1") ||
        ...
        strstr(saved_command_line,"board_id=S19612KA1") ||
        strstr(saved_command_line,"board_id=S19612LA1")) {
        pr_info("%s, chip id of marlin3 lite is %d, open %s\n",
            __func__, wcn_get_chip_type(), SYSTEM_WIFI_612CE_CONFIG_FILE);
        return wifi_nvm_parse(SYSTEM_WIFI_612CE_CONFIG_FILE, (void *)p);
    } else if (strstr(saved_command_line,"board_id=S19612GA1") ||
