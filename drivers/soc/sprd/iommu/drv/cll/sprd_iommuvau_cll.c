...
static u32 sprd_iommuvau_cll_init(struct sprd_iommu_init_param *init_param,
...
static u32 sprd_iommuvau_cll_uninit(sprd_iommu_hdl  iommu_hdl)
{
	struct sprd_iommu_data *iommu_data = NULL;
	struct sprd_iommuvau_priv *iommu_priv = NULL;
	u32 iommu_id;
	...
	mmu_vau_enable(iommu_priv->mmu_reg_addr, iommu_id, 0);
	if (iommu_priv->pagt_base_phy_ddr > 0)
...
static void sprd_iommuvau_flush_pgt(ulong ppn_base,
...
static u32 sprd_iommuvau_cll_enable(sprd_iommu_hdl iommu_hdl)
{
	struct sprd_iommu_data *iommu_data = NULL;
	struct sprd_iommuvau_priv *iommu_priv = NULL;
	u32 iommu_id;
	...
	mmu_vau_first_vpn(iommu_priv->mmu_reg_addr, iommu_id,
		iommu_priv->vpn_base_addr);
	mmu_vau_first_ppn(iommu_priv->mmu_reg_addr, iommu_id, pgt_addr_phy);
	...
	if (iommu_priv->mini_ppn1 > 0)
		mmu_vau_mini_ppn1(iommu_priv->mmu_reg_addr, iommu_id,
	...
	if (iommu_priv->mini_ppn2 > 0)
		mmu_vau_mini_ppn2(iommu_priv->mmu_reg_addr, iommu_id,
...
static u32 sprd_iommuvau_cll_disable(sprd_iommu_hdl  iommu_hdl)
{
	struct sprd_iommu_data *iommu_data = NULL;
	struct sprd_iommuvau_priv *iommu_priv = NULL;
	u32 iommu_id;
	...
	mmu_vau_first_vpn(iommu_priv->mmu_reg_addr, iommu_id, 0);
	mmu_vau_first_ppn(iommu_priv->mmu_reg_addr, iommu_id, 0);
	mmu_vau_default_ppn(iommu_priv->mmu_reg_addr, iommu_id, 0);

	if (iommu_priv->mini_ppn1 > 0)
		mmu_vau_mini_ppn1(iommu_priv->mmu_reg_addr,
			iommu_id, 0);
	...
	if (iommu_priv->mini_ppn2 > 0)
		mmu_vau_mini_ppn2(iommu_priv->mmu_reg_addr,
				iommu_id, 0);
...
static u32 sprd_iommuvau_cll_map(sprd_iommu_hdl  iommu_hdl,
				struct sprd_iommu_map_param *map_param)
{
	...
	if (map_param->sg_table == NULL) {
		...
		memset32((void *)(iommu_priv->ppn_base_addr +
				  vir_base_entry * 4),
	...
	if (iommu_priv->pagt_base_phy_ddr == 0)
		sprd_iommuvau_flush_pgt(iommu_priv->ppn_base_addr,
				       vir_base_entry,
				       total_page_entries);
...
static u32 sprd_iommuvau_cll_unmap(sprd_iommu_hdl iommu_hdl,
...
static u32 sprd_iommuvau_cll_unmap_orphaned(sprd_iommu_hdl iommu_hdl,
			struct sprd_iommu_unmap_param *unmap_param)
{
	u32 valid_page_entries = 0;
	ulong vir_base_entry = 0;
...
static u32 sprd_iommuvau_cll_suspend(sprd_iommu_hdl iommu_hdl)
{
	return 0;
}

static u32 sprd_iommuvau_cll_resume(sprd_iommu_hdl  iommu_hdl)
{
	return 0;
}

static u32 sprd_iommuvau_cll_release(sprd_iommu_hdl  iommu_hdl)
{
	struct sprd_iommu_data *iommu_data = NULL;
	struct sprd_iommuvau_priv *iommu_priv = NULL;
...
static u32 sprd_iommuvau_cll_reset(sprd_iommu_hdl  iommu_hdl, u32 channel_num)
{
	struct sprd_iommu_data *iommu_data = NULL;
	struct sprd_iommuvau_priv *iommu_priv = NULL;
...
static u32 sprd_iommuvau_cll_set_bypass(sprd_iommu_hdl iommu_hdl,
					bool vaor_bp_en)
{
	struct sprd_iommu_data *iommu_data = NULL;
	struct sprd_iommuvau_priv *iommu_priv = NULL;
	...
	mmu_vau_vaout_bypass_enable(iommu_priv->mmu_reg_addr,
				    iommu_priv->iommu_id, vaor_bp_en);
	return 0;
}

static u32 sprd_iommuvau_cll_virt_to_phy(sprd_iommu_hdl iommu_hdl,
			u64 virt_addr, u64 *dest_addr)
{
	...
	entry_index = VIR_TO_ENTRY_IDX(virt_addr,
				iommu_priv->vpn_base_addr);
	phy_page_addr = mmu_vau_read_page_entry(iommu_priv->ppn_base_addr,
					    entry_index);
...
struct sprd_iommudrv_ops iommuvau_drv_ops = {
