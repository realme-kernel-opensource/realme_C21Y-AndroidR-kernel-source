...
static struct pdm_info sharkl5_pdm_info[] = {
	{
		.addr_offset = 0x00BC,
		.pwd_bit_width = 8,
		.bit_index = {0, 8, 16, 24, MAX_STATES_NUM_PER_REG},
		.pdm_name = {"PD_AP_SYS", "PD_AP_VSP", "PD_CDMA_SYS", NULL}
	},
	{
		.addr_offset = 0x00C0,
		.pwd_bit_width = 8,
	...
	},
	{
		...
		.pwd_bit_width = 8,
		.bit_index = {0, 8, 16, 24, MAX_STATES_NUM_PER_REG},
		.pdm_name = {"PD_AUDCP_AUDDSP", "PD_GPU_TOP",
...
static struct reg_check sharkl5_ap_ahb_reg[] = {
	{
		.addr_offset = 0x0000,
		.value_mask = 0xBB,
		.expect_value = 0x80,
...
static struct reg_check sharkl5_ap_apb_reg[] = {
	{
		.addr_offset = 0x0000,
	...
	},
	{
		...
		.value_mask = 0x07FC0,
		.expect_value = 0x00,
		.preg_name = "REG_AP_APB_APB_MISC_CTRL"
	}
};

static struct reg_check sharkl5_pmu_apb_reg[] = {
	{
		.addr_offset = 0x00C8,
...
static struct reg_check sharkl5_aon_apb_reg[] = {
	{
		.addr_offset = 0x0000,
		.value_mask = 0xA0,
		.expect_value = 0x00,
...
static struct intc_info sharkl5_pintc_info[] = {
	{
		.addr_offset = 0x0000,
...
static void sharkl5_output_2nd_irq_source(void *pentry, u32 hw_irq_nr)
{
	struct power_debug *pdbg = (struct power_debug *)pentry;

	if (pdbg_sharkl5.pdbg != pdbg)
		return;
...
static struct power_debug_desc sharkl5_pdbg_desc = {
	...
	.pmu_pdm_num = sizeof(sharkl5_pdm_info)/sizeof(struct pdm_info),
	.ap_ahb_reg_num = sizeof(sharkl5_ap_ahb_reg)/sizeof(struct reg_check),
	.ap_apb_reg_num = sizeof(sharkl5_ap_apb_reg)/sizeof(struct reg_check),
	.pmu_apb_reg_num = sizeof(sharkl5_pmu_apb_reg)/sizeof(struct reg_check),
	.aon_apb_reg_num = sizeof(sharkl5_aon_apb_reg)/sizeof(struct reg_check),
	.ap_intc_num = sizeof(sharkl5_pintc_info)/sizeof(struct intc_info),
	.irq_mask = {0, 0, 0, 0, 0, 0x2000, 0, 0},

	.ppdm_info = sharkl5_pdm_info,
	.ap_ahb_reg = sharkl5_ap_ahb_reg,
	.ap_apb_reg = sharkl5_ap_apb_reg,
	.pmu_apb_reg = sharkl5_pmu_apb_reg,
	.aon_apb_reg = sharkl5_aon_apb_reg,
	.pintc_info = sharkl5_pintc_info,
	.log_2nd_irq_source = sharkl5_output_2nd_irq_source
};
...
static int sprd_pdbg_sharkl5_probe(struct platform_device *pdev)
{
	...
	pcfg = sprd_pdbg_parse_cfg(&pdev->dev, sharkl5_pdbg_desc.ap_intc_num);
	if (IS_ERR(pcfg))
		return (int)pcfg;
...
static int sprd_pdbg_sharkl5_remove(struct platform_device *pdev)
{
	...
	devm_kfree(&pdev->dev, pdbg_sharkl5.pcfg);
	pdbg_sharkl5.pcfg = NULL;
	pdbg_sharkl5.pdev = NULL;

	return 0;
}

static const struct of_device_id sprd_pdbg_sharkl5_of_match[] = {
	{
		.compatible = "sprd,power-debug-sharkl5",
	},
	{},
};
MODULE_DEVICE_TABLE(of, sprd_pdbg_sharkl5_of_match);

static struct platform_driver sprd_pdbg_sharkl5_driver = {
	.probe = sprd_pdbg_sharkl5_probe,
	.remove = sprd_pdbg_sharkl5_remove,
	.driver = {
		.name = "sprd-powerdebug",
		.of_match_table = sprd_pdbg_sharkl5_of_match,
	},
};
...
MODULE_DESCRIPTION("sprd sharkl5 power debug driver");
