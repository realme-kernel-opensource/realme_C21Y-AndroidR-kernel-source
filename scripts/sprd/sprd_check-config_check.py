...
# **** check base_arm/arm64.config(if exists) vs plat(arm/arm64)
        if arch_base in d_baseconfig:
            for config in d_baseconfig[arch_base]:
                if config not in d_defconfig[plat]:
                    if d_baseconfig[arch_base][config] == 'n':
                        print("WARNING: missing configuration: base:" + config + " " + d_baseconfig[arch_base][config])
                    else:
                        print("ERROR: missing configuration: base:" + config + " " + d_baseconfig[arch_base][config])
                elif config in d_defconfig[plat]:
                    if d_baseconfig[arch_base][config] != d_defconfig[plat][config]:
                        print("ERROR: different configuration: base:" + config + " " + d_baseconfig[arch_base][config] + "\t"\
                            + plat + ":" + config + " " + d_defconfig[plat][config])
# **** check base.config vs plat(no matter what arch)
        for config in d_baseconfig['base']:
            if config not in d_defconfig[plat]:
